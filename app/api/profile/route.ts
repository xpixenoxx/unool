...
export async function PUT(request: NextRequest) {
  // For profile creation/update, we need to check if it's a new profile (create) or existing (update)
  // We'll use multi-plan enforcement for both cases
  return planEnforcement.createProfile(request, async (request) => {
    try {
      const auth = await getCurrentAuth(request);
      if (!auth) {
        return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
      ...
      if (profile) {
        // Update existing - no plan limit for updates
        ...
        // Create new - already checked via plan enforcement
...
export async function POST(request: NextRequest) {
  // Alias to PUT for creation - will check create_profile limit
  return PUT(request);
}
